...
xlsfile1 = r'F:/testexcel/testorderres.xls'
reload(sys)
sys.setdefaultencoding('utf-8')

    
def testorderreserveexcel(xlsfile=xlsfile1):
     
    title='START testorderreserve'
    printtitle(title)
    ...
    for i in range(1,nrows):
        carId = api_sheet.cell_value(i,0)   # 分别迭代表格中的client_id，password，sign和ex_code
        #print mobile
        rentType = api_sheet.cell_value(i,1)
        #print password
        takeTime = api_sheet.cell_value(i,2)
        returnTime= api_sheet.cell_value(i,3)
        takeStationId= api_sheet.cell_value(i,4)
        returnStationId= api_sheet.cell_value(i,5)
        exInsuranceTag = api_sheet.cell_value(i,6)
        uid= api_sheet.cell_value(i,7)
        token= api_sheet.cell_value(i,8)
        secretkey = api_sheet.cell_value(i,9)
        
        ex_code = api_sheet.cell_value(i,10)
        ex_message=api_sheet.cell_value(i,11)
...
if __name__=="__main__":
    testorderreserveexcel(xlsfile1)
